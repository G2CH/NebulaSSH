...
export default function App() {
  ...
  const handleConnect = React.useCallback(async (server: Server) => {
    ...
    if (server.protocol === 'local') {
      ...
      // SSH connection - will get remote home directory after connection
      defaultDir = '/'; // Temporary, will be updated
    }
  ...
  const handleCloseSession = React.useCallback((sessionId: string, e?: React.MouseEvent) => {
    e?.stopPropagation();

    setSessions(prev => {
      const newSessions = prev.filter(s => s.id !== sessionId);

      // Calculate new active session if the closed one was active
  ...
  const handleDeleteServer = React.useCallback(async (serverId: string | number) => {
    ...
    if (confirm(t('common.confirm_delete') || 'Are you sure you want to delete this connection?')) {
      try {
        ...
        // Close any active sessions for this server
        // Use ref to get current sessions without dependency
        const currentSessions = sessionsRef.current;
        const serverSessions = currentSessions.filter(s => s.serverId === serverId);

        // We need to close them one by one, but handleCloseSession updates state.
        // It's better to do a bulk update here.
        if (serverSessions.length > 0) {
          setSessions(prev => {
            const newSessions = prev.filter(s => s.serverId !== serverId);

            // If active session was removed, update active ID
            if (activeSessionIdRef.current && serverSessions.some(s => s.id === activeSessionIdRef.current)) {
              const newActiveId = newSessions.length > 0 ? newSessions[newSessions.length - 1].id : null;
              setActiveSessionId(newActiveId);
            }
